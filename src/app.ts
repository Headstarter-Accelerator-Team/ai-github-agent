...
const getChangesPerFile = async (payload: WebhookEventMap["pull_request"]) => {
  try {
    ...
    if (!files || files.length === 0) {
      console.log("No files found in pull request");
    } else {
    ...
    console.error("Error getting PR files:", exc);
    // Log specific error details that might help debugging
    if (exc.status) console.error(`Status: ${exc.status}`);
    if (exc.message) console.error(`Message: ${exc.message}`);
    throw exc; // Re-throw instead of returning empty array
  }
};
